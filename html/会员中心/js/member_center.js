	...
	$.module({
		...
		表单验证:{
			...
			rules:{
				...
				,3:{
					$reference:$(".login_con ul li :text,.login_con ul li :password").eq(2),
			...
			init:function(m){
				...
				$("input.button").click(function(){
					if($(this).hasClass("disable_false")){
						...
						var $select=$("select");
						if($select.val()==0){
							$select.closest("li").find(".error").show();
						}
