$(function(){
	...
	//修复男 女 及其checkbox样式
	var $li=$(".set_content:first ul li").eq(2);
	var li_html=$li.html();
	...
	//年月日选择
	var $li=$(".set_content:first ul li").eq(3);
	// var li_html=$li.html();
	...
	var $inputs=$(".set_content :text,.set_content textarea");
		$.each({
			...
			0:{
				...
				gt_max:"昵称过长(最多16个字符)",
				reg:/^[A-Za-z0-9_\-\u4e00-\u9fa5]+$/,
				format_error:"昵称格式错误（只允许字母数字下划线汉字）"
			},
			2:{
				max:100,
			...
			5:{
				null_value:"请输入毕业学校",
				min:2,
				lt_min:"毕业学校过短(至少2个字符)",
				max:100,
				gt_max:"毕业学校过长(最多100个字符)",
				reg:/^[A-Za-z0-9_\-\u4e00-\u9fa5]+$/,
				format_error:"毕业学校格式错误（只允许字母数字下划线汉字）"
			},
			6:{
				null_value:"请输入签名",
				min:2,
	...
	$inputs.eq(6).textarea_words_limit({
		$words_msg_ti_shi_obj:(function(){
			var html=$inputs.eq(6).next().html();
			var limit=parseInt(html.substring(html.indexOf("/")+1));
			return $inputs.eq(6).next().html("<label>0</label>/<label>"+limit+"</label>").find("label").first()
		})(),
		words_num_limit:parseInt($inputs.eq(6).next().find("label").last().html()),
